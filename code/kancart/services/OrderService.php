...
class OrderService extends BaseService {
    ...
    public function getOrderInfos(array $parameter) {
        ...
        $orderStatus = ServiceFactory::factory('Store')->getOrderStatauses();
        foreach ($orders as $order) {
            ...
            $orderItem['order_status'] = $orderStatus;
    ...
    public function getPaymentOrderInfoById($orderId, $tx = '') {
        ...
        if ($order) {
            ...
            $total = $order['total'] * $order['currency_value'];
    ...
    public function getPaymentAddress($address) {
        $addr = array(
            ...
            'country_id' => $address['shipping_country_id'],
            'zone_id' => $address['shipping_zone_id'], //1
            'zone_name' => $address['shipping_zone'], //2
            'state' => '', //3
            'address1' => $address['shipping_address_1'],
    ...
    public function getPaymentPriceInfos($order) {
        ...
        foreach ($rows as $row) {
            $info[] = array(
                'type' => $row['code'],
                'home_currency_price' => $row['value'] * $order['currency_value']
            );
        }
    ...
    public function getPaymentOrderItems($order) {
        ...
        foreach ($products as $product) {
            $items[] = array(
                ...
                'category_name' => '',
                'home_currency_price' => $product['price'] * $order['currency_value'],
    ...
    public function getOrderItems(array $order) {
        ...
        foreach ($query->rows as $row) {
            ...
            $items[] = array('order_item_id' => '',
                ...
                'qty' => $row['quantity'],
                'price' => $row['price'] * $order['currency_value'],
                'final_price' => $row['price'] * $order['currency_value'],
                'item_tax' => $row['tax'],
                'shipping_method' => '',
                'post_free' => false,
                'virtual_flag' => false);
